---
title: "Dealflow into Second Brains"
lede: "How to pipe raw dealflow — intake forms, inbound, sourcing notes — into a second brain that actually compounds. Toby and Marcus demo the OpenClaw → Obsidian and intake → Notion pipelines they've been building for deal triage and pipeline insight."
date_scheduled: 2026-06-24T09:00:00-07:00
date_posted: 2026-06-21
durationMinutes: 60
hosts:
  - Michael Staton
presenters:
  - Michael Staton, FullStack VC
  - Marcus Brotman, Launch
  - Toby Rush, Ideem
presenterDetails:
  - name: Michael Staton
    firm: Lossless Group / FullStack VC
    role: Agent Conductor
    headshot: /images/participant-headshots/headshot__Michael-Staton.jpeg
    linkedin: https://www.linkedin.com/in/michaelstaton/
    topic: Obsidian & Markdown Notebooks for Dummies.
  - name: Marcus Brotman
    firm: Launch
    headshot: /images/presenter-headshots/headshot__Marcus-Brotman.jpeg
    linkedin: https://www.linkedin.com/in/marcus-brotman/
    topic: Intake forms + OpenClaw + Notion for better deal-pipeline insights.
  - name: Toby Rush
    firm: Ideem
    role: Founder, CEO
    headshot: /images/participant-headshots/headshot__Toby-Rush.jpeg
    linkedin: https://www.linkedin.com/in/tobiasrush/
    topic: OpenClaw + Obsidian for deal triage and vigilant pipeline management.
partnerMark:
  name: Kauffman Fellows
  light: /trademark__Kauffman-Fellows--Light-Mode.svg
  dark: /trademark__Kauffman-Fellows--Dark-Mode--Lightest.png
  vibrant: /trademark__Kauffman-Fellows--Vibrant.svg
  url: https://www.kauffmanfellows.org/
classifiers:
  - All-Hands
tags:
  - Agentic-AI
  - Venture-Capital
  - Dealflow
  - Second-Brain
  - Community
og_image: /og-default.png
rsvpUrl: https://kf.events/3ReV0eh
---

This month's Agentic VC Dojo is themed **Dealflow into Second Brains** — getting the messy front of the funnel (intake forms, inbound, sourcing notes, founder calls) into a system that compounds instead of rotting in an inbox.

A primer up front, then Marcus and Toby return to give the demos they had queued up for earlier sessions but didn't get to present:

- **Michael Staton (FullStack VC)** — Obsidian & Markdown Notebooks for Dummies. The ground-floor primer: what a markdown second brain is and how to start one.
- **Marcus Brotman (Launch)** — Intake forms + OpenClaw + Notion for better deal-pipeline insights.
- **Toby Rush (Ideem)** — OpenClaw + Obsidian for deal triage and vigilant pipeline management.

Two takes on the same problem — one routing into **Notion**, one into **Obsidian** — bookended by a from-scratch primer, so you can see how the second-brain choice shapes the workflow whether you're starting today or already deep in.

## When

Wednesday, June 24, 2026 · 9:00 AM Pacific Time

## Format

- Dojo convening + framing
- Markdown second-brain primer (Michael)
- Two lightning demos (Marcus, Toby)
- Live polls
- Town hall — bring a dealflow-capture workflow you're wrestling with
- Q&A and next steps

## Who this is for

VCs from analyst to partner. No prior AI experience required — whether your dealflow lives in a spreadsheet, a CRM, or your head, come see how others are wiring it into a second brain.
