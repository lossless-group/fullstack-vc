// Mode Switcher Utility — supports the firm-wide three-mode contract:
// 'light' | 'dark' | 'vibrant'.
// Extended from hypernova-site's two-mode version (which currently only handles
// light/dark). See blueprint: Maintain-Themes-Mode-Across-CSS-Tailwind.md §4.
...
export class ModeSwitcher {
  ...
  /**
   * Cycle through modes: light → dark → vibrant → light.
  ...
  setMode(mode) {
    if (!VALID_MODES.includes(mode)) {
      console.warn(`Invalid mode: ${mode}. Valid: ${VALID_MODES.join(', ')}`);
...
if (typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => {
    modeSwitcher.applyMode(modeSwitcher.getStoredMode() || modeSwitcher.getSystemPreference(), true);
  });
}
