...
panels:
  - n: 1
    ...
    subline: "April 29 launch: 60 registered · 40 surveyed (67%) · 37 firms · 73% active rate"
    key_numbers:
      registered: 60
      surveyed: 40
  ...
  - n: 2
    ...
    subline: "Top: people experimenting. Bottom: only 1 person has consistent firm-wide workflows with big impact."
    highlights:
  ...
  - n: 3
    ...
    subline: "ChatGPT / Claude / Perplexity: 33 of 40 already using. Agent workflow builders: 13 of 40 not yet touching."
    highlights:
  ...
  - n: 4
    ...
    data_path: "src/data/webinar-survey/2026-05-27_combined-firms.json (generated by scripts/build-combined-firms.ts)"
    headline_overlay: "And here's the network — repeats in accent."
    subline: "56 firms across the April 29 launch and the May 27 all-hands. The 8 that came back are highlighted."
    ...
    notes:
      ...
      - "May 27 added 19 new firms after deduping 8 firm-level overlaps and excluding non-firm domains (gmail / intel / okta)."
      - "Firm-level repeats (verified by the merge script): Cherry Rock Capital, Domo VC, Feature VC, iMena, Last Mile Ventures, M25 Ventures, Mercury Fund, Prose Ventures."
      - "Individual-level repeat tracking was considered but skipped: only ~4 named individuals appear in both sessions, which is too small a sample to highlight visually without looking sparse. Firm-level (8) is the cleaner signal."
    data_combine_recipe: |
      Run scripts/build-combined-firms.ts (via `pnpm build:combined-firms`) to regenerate.
      Inputs:
        - src/data/webinar-survey/2026-04-29_agentic-vc-dojo-launch.json → $.firms (37 entries)
        - context-v/extra/2026-05-27_ ATTENDEES.csv → email-column domains, excluding gmail.com / intel.com / okta.com
      Output:
        - src/data/webinar-survey/2026-05-27_combined-firms.json — extends the April $.firms shape with `source: 'april-only' | 'may-only' | 'both'` and `repeat: boolean`.
      Section__WebinarFirmsRepresented consumes the JSON; a small prop addition lets it pick up the `repeat` flag and apply accent styling.
    visual_emphasis: |
      Two states for a firm chip in the grid/marquee:
      1. NEW (one session only) — default chip, `--color-surface` background, `--color-text` text.
      2. REPEAT FIRM (domain in both sessions) — `--color-accent` background or accent border, depending on what reads better at deck scale.
  ...
  - n: 5
    ...
    subline: "Free-response answers to 'what do you most want to get out of this session?' — lightly edited, anonymized."
    quote_count: 4
    sample_personas:
      - "A solo GP — best practices to get 80% there with 20% effort"
      - "A frequent-experimenter solo partner — understanding output quality"
      - "A research-driven seed investor — architecture, tools, and best practices to set up securely"
  ...
  - n: 6
    ...
    quote_count: 9
    featured_personas:
      - "An emerging-manager GP — automating the horrible parts of being an emerging manager. Maybe a Kauffman toolset."
