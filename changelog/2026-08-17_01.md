...
lede: "Screenshots of a setup flow are a sequence, and we had no way to say so in markdown. `:::image-carousel` now does, in four variants split as ImageCarousel--{Variant} siblings. Building the chronological-sort default surfaced that the image-prep timestamp records when a file was processed, not when the screen was captured — which is fine until you re-upload one image mid-sequence, as we just did."
publish: true
authors:
  - Michael Staton
...
- Fix the dots list styling: `.docs-prose ol { list-style: decimal }` and `.docs-prose a` out-specify the single-class carousel rules, so the nav renders with list markers and underlines inside prose.
